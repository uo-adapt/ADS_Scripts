...
source /projects/adapt_lab/shared/ADS/Scripts/sMRI/SetUpFreeSurfer_w2.sh

echo -e "\nSetting up AFNI"

module load afni

rsfMRIdir="/projects/adapt_lab/shared/ADS/data/BIDS_data/derivatives/rsfMRI_preproc_wave2"
...
for label in $lhlabels; do
 3dAllineate -source $SUBJECTS_DIR/"${SUBID}"_ses-wave2.long.$SUBID/mri/fromannots/"${label}".nii.gz -master $rsfMRIdir/"${SUBID}"/"${SUBID}".results/*r01.volreg.nii.gz -final NN -1Dparam_apply '1D: 12@0'\' -prefix $rsfMRIdir/"${SUBID}"/"${SUBID}".results/alignedrois/"${label}"_aligned.nii.gz 
...
3dAllineate -source $SUBJECTS_DIR/"${SUBID}"_ses-wave2.long.$SUBID/SUMA/aparc.a2009s+aseg.nii.gz -master $rsfMRIdir/"${SUBID}"/"${SUBID}".results/*r01.volreg.nii.gz -final NN -1Dparam_apply '1D: 12@0'\' -prefix $rsfMRIdir/"${SUBID}"/"${SUBID}".results/aparc.a2009s+aseg_aligned.nii.gz 

3dAllineate -source $SUBJECTS_DIR/"${SUBID}"_ses-wave2.long.$SUBID/SUMA/aparc+aseg.nii.gz -master $rsfMRIdir/"${SUBID}"/"${SUBID}".results/*r01.volreg.nii.gz -final NN -1Dparam_apply '1D: 12@0'\' -prefix $rsfMRIdir/"${SUBID}"/"${SUBID}".results/aparc+aseg_aligned.nii.gz 
