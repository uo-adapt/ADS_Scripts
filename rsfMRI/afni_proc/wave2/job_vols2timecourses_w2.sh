...
for label in $lhlabels; do
 3dAllineate -source $SUBJECTS_DIR/"${SUBID}"_ses-wave2.long.$subj/mri/fromannots/"${label}".nii.gz -master $rsfMRIdir/"${SUBID}"/"${SUBID}".results/*r01.volreg.nii.gz -final NN -1Dparam_apply '1D: 12@0'\' -prefix $rsfMRIdir/"${SUBID}"/"${SUBID}".results/alignedrois/"${label}"_aligned.nii.gz 
...
# rm $SUBJECTS_DIR/"${SUBID}"_ses-wave2.long.$subj/mri/fromannots/"${label}".nii.gz
...
3dAllineate -source $SUBJECTS_DIR/"${SUBID}"_ses-wave2.long.$subj/SUMA/aparc.a2009s+aseg.nii.gz -master $rsfMRIdir/"${SUBID}"/"${SUBID}".results/*r01.volreg.nii.gz -final NN -1Dparam_apply '1D: 12@0'\' -prefix $rsfMRIdir/"${SUBID}"/"${SUBID}".results/aparc.a2009s+aseg_aligned.nii.gz 

3dAllineate -source $SUBJECTS_DIR/"${SUBID}"_ses-wave2.long.$subj/SUMA/aparc+aseg.nii.gz -master $rsfMRIdir/"${SUBID}"/"${SUBID}".results/*r01.volreg.nii.gz -final NN -1Dparam_apply '1D: 12@0'\' -prefix $rsfMRIdir/"${SUBID}"/"${SUBID}".results/aparc+aseg_aligned.nii.gz 
