...
epidewarp.fsl --mag "${workspace}"/fmap/"${subid}"_ses-wave2_magnitude2.nii.gz --dph "${workspace}"/dwi/fieldmap.nii.gz --epi "${workspace}"/dwi/"${subid}"_ses-wave2_dwi.nii.gz --tediff 2.46 --esp 0.00046484061 --vsm vox_shift_map.nii.gz --epidw test.nii.gz
