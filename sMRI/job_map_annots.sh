...
for label in $lhlabels; do
	mri_label2vol --label $SUBJECTS_DIR/$SUBID/label/fromannots/"${label}".label --subject $SUBID --hemi lh --identity --temp $SUBJECTS_DIR/$SUBID/mri/orig.mgz --o $SUBJECTS_DIR/$SUBID/mri/fromannots/"${label}".nii.gz --proj frac 0 1 0.01
 ...
 for label in $rhlabels; do
	mri_label2vol --label $SUBJECTS_DIR/$SUBID/label/fromannots/"${label}".label --subject $SUBID --hemi rh --identity --temp $SUBJECTS_DIR/$SUBID/mri/orig.mgz --o $SUBJECTS_DIR/$SUBID/mri/fromannots/"${label}".nii.gz --proj frac 0 1 0.01
	mri_binarize --dilate 1 --erode 1 --i $SUBJECTS_DIR/$SUBID/mri/fromannots/"${label}".nii.gz --o $SUBJECTS_DIR/$SUBID/mri/fromannots/"${label}"_bi.nii.gz --min 1
